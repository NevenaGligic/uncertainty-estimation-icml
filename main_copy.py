...
from src.models.nu_EDL import nu_EDL
...
from src.experiments_copy import run_benchmark
...
def main():
    parser = argparse.ArgumentParser(description="Unified Benchmarking Script.")
    parser.add_argument('--dataset', type=str, choices=['mnist', 'cifar10'], default='mnist', help="ID dataset for training and comparison.")
    parser.add_argument('--model', type=str, choices=['nu_edl', 'edl', 'redl', 'daedl', 'postnet'], default='nu_edl', help="Model to use for training and evaluation.")
    ...
    if args.edl_path is None: args.edl_path = f"{save_dir}/{args.dataset}_EDL_model_{seed_suffix}.pth"

    if args.maf_path is None:
        if args.dataset == 'mnist':
            args.maf_path = f"{save_dir}/{args.dataset}_MAF_model_{seed_suffix}.pth"
